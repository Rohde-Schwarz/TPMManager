...
% or something similar.
 \logo{\includegraphics[width=6cm]{cover} } %%%%%%%%%%%%%%%%%%%%%%%%%%%%<<<<<<<<<-------------------------<<<<<<<<%%%%%%%%%
% Title
\title{The TPM Manager}
\subtitle{Requirements, Design, and Implementation}
% Author or list of authors
%\author{Anoosheh Zaerin\\\url{anoosheh.zaerin@rub.de}}  % Use \and to add more authors...
...
\newcommand{\System}{MyProduct}  % Replace by product name or sth. similar
...
\begin{figure}[h]
 \centering
 \includegraphics[width=1\textwidth]{images/graphic3.jpg}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{images/graphic4.jpg}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{images/graphic5.jpg}
% graphic4.jpg: 751x445 pixel, 762dpi, 2.50x1.48 cm, bb=0 0 71 42
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic6.jpg}
...
\ucnormal 
 ...
 \item TPM Manager displays a dialog window to choose, the type of SRK password
 \item User chooses the default choice (set SRK to \lstinline'WELL_KNOWN_SECRET')
 \item TPM Manager set SRK password to \lstinline'WELL_KNOWN_SECRET'
 \item TPM Manager displays a dialog to choose the owner password
 \item User chooses the owner password and confirm it
 \item TPM Manager set the password and show in a dialog if the owner password is successfully set or not
\ucendflow 
\ucalternate
 \item User clicks on the ''Ownership`` tab
 \item User hits the ''Take`` button
 \item TPM Manager displays a dialog window to choose, the type of SRK password
 \item User chooses the second choice (set SRK manually)
 \item TPM Manager displays a dialog to choose the SRK password
 \item User chooses SRK password and confirm it
 \item TPM Manager displays a dialog to choose the owner password
 \item User chooses the owner password and confirm it
 \item TPM Manager sets the password and show in a dialog if the owner and SRK password are successfully set or not
\ucendflow 
\end{usecase}
\clearpage
...
\ucnormal 
 ...
 \item TPM Manager checks the owner password
 \item If owner password is correct the next dialog from TPM Manager asks for new password
 \item If owner password is correct a warning dialog will be displayed
\ucendflow 
\end{usecase}
\clearpage
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic7.jpg}
...
\ucnormal 
 ...
 \item Owner authorization
 \item Confirm create the archive
\ucendflow
\end{usecase}
\clearpage
...
\ucnormal 
 ...
 \item Owner selects archive file
 \item Owner authorization
 \item Confirm load SRK archive
\ucendflow
\end{usecase}

\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic8.jpg}
...
\ucdesc In this Section can TPM can be switched to enabled or disabled mode deactivated mode
\ucactors  Owner
\ucinclude 
   \begin{compactitem}
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic9.jpg}
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic10.jpg}
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic11.jpg}
...
\begin{usecase}{Enable Critical Functions}{critical}
\ucdesc The ''Advanced'' section needs to be enabled explicitly  by the user, because the provided features that can perform
\ucactors Owner
\end{usecase}

\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic12.jpg}
 \caption{Tab dialog to enable critical features \ucref{critical}}
\end{figure}
\clearpage
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic13.jpg}
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic14.jpg}
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.4\textwidth]{images/graphic22.jpg}
...
While the GUI layer provides the user interface components, based on widgets of the KDE\footnote{http://www.kde.org/} and
Qt\footnote{http://www.troll.no/} frameworks, the MicroTSS layer offers a simple, object-oriented interface to the functions
...
the Qt-based class, \lstinline'TPM_ManagerWidget'. Its base class, \lstinline'TPM_ManagerWidgetBase', is created by the
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.8\textwidth]{images/graphic19.jpg}
...
of the class \lstinline'TPM_ManagerWidget' is invoked (step 1). Here, two password dialogs are created for entering
...
\begin{figure}[h]
 \centering
 \includegraphics[width=0.9\textwidth]{images/graphic20.jpg}
...
The ''Enable``/''Disable`` button on the ''Operational Mode`` page of the TPM Manager GUI will be only active if the ownership of the TPM is done successfully. The caption of the button is set by class \lstinline'TPM_ManagerWidget' correspondence to the current status of the TPM. If the TPM is enabled the button is set to Disable and if the TPM is disabled the caption of the button will be Enable.

After the user clicks on the ''Enable``/''Disable`` button on the TPM Manager dialog, the appropriate method \lstinline'slotSetEnabledDisabled()' of the class \lstinline'TPM_ManagerWidget' is called. This method invokes a password dialog that asks for owner password. After the correct owner password is given by the user, the appropriate funtions \lstinline'setDisabled()'/\lstinline'setEnabled()' will be called, that are methods of the class \lstinline'TPM'. Lastly, a dialog informs the user about the result of the Enable/Disable operation.

\subsection{Use Case PCRs}
Choosing ''Info'' option from the left menu and then choosing the ``PCRs'' tab the function of \lstinline'TPM_ManagerWidget' will be called. It shows the current values of the PCRs only if the TPM is activated. The values are regularely updated by a signal coming from the system timer every 1 second. 
...
Since the TPM Manager is based on some KDE widgets, corresponding header and library files of KDE and Qt should be in the library path. On some linux distributions you have to install the developer version of Qt to have the header files used by TPM Manager.
...
The TPM Manager has been successfully compiled under KDE 3.5 and Qt3.


\section{Installation and Configuration}
The TPM Manager is hosted on sourceforge at \url{http://sourceforge.net/projects/tpmmanager/}. The software is based on automake/autoconf; therefore, it can be configured and installed as described bellow.
...
\begin{figure}[h]
 \centering
   \includegraphics[width=0.8\textwidth]{images/tpmmanager.jpg}
...
\begin{figure}[h]
 \centering
   \includegraphics[width=0.8\textwidth]{images/status3.jpg}
...
\begin{figure}[h]
 \centering
   \includegraphics[width=0.8\textwidth]{images/take.jpg}
   \caption{Take ownership}
\label{fig:take}
\end{figure}
\begin{figure}[h]
 \centering
   \includegraphics[width=0.6\textwidth]{images/srk.jpg}
   \caption{Dialog for setting SRK}
\label{fig:srk}
\end{figure}
\begin{figure}[h]
 \centering
   \includegraphics[width=0.8\textwidth]{images/take2.jpg}
...
\begin{figure}[h]
 \centering
   \includegraphics[width=0.8\textwidth]{images/enable.jpg}
...
\section{Open Issues}
\begin{itemize}
 \item Make KDE-Support optional (using a \emph{./configure} option)
 \item Make the TPM Manager optionally a KConfig instance
\end{itemize}
